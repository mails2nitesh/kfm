...
function kfm_modal_open(form,title,actions){
	...
	if(browser.isIE)setCss(body,'overflow:hidden');
	{ // shader
		setCss(shader,'background:#fff;opacity:.5;position:'+pos+';top:'+scrollAt.y+'px;left:'+scrollAt.x+'px;zIndex:2;width:'+a.x+'px;height:'+a.y+'px');
		body.addEl(shader);
	}
	{ // wrapper
		var wrapper=newEl('div','formWrapper');
		var h2=newEl('h2',0,0,title,0,'float:left');
		setCss(form,'position:relative;margin:0;textAlign:left;padding:0;clear:left');
		...
		setCss(row,'background:#eee;borderTop:1px solid #ddd;textAlign:right;padding:2px;zIndex:3');
		body.addEl(wrapper);
		setCss(wrapper,'width:'+(form.offsetWidth+10)+'px');
		var w=wrapper.offsetWidth;
		if(w<200||w>a.x*.9){
			w=w<200?200:parseInt(a.x*.9);
			setCss(wrapper,'width:'+w+'px');
		}
		...
		if(parseFloat(h)>parseFloat(a.y*.9)){
			...
			setCss(form,'margin:0 auto;overflow:auto;height:'+h3+'px;maxHeight:'+h3+'px');
		}else{
			var h3=h-row.offsetHeight-h2.offsetHeight-q;
			setCss(form,'overflow:auto;width:100%;maxHeight:'+h3+'px');
		}
		setCss(wrapper,'position:'+pos+';left:'+(scrollAt.x+a.x/2-w/2)+'px;top:'+(scrollAt.y+a.y/2-h/2)+'px;background:#fff;zIndex:3;border:1px solid #000');
	}
}
